...
When the chain is stationary and posterior variance is finite, maximising the ESEJD is equivalent to minimising a weighted sum of the autocorrelations up to a lag value \(l - 1\). We can observe how autocorrelation varies against different rate parameter choices (Figure \ref{fig:org2e135b6}). 


\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{Theoretical_Basis/2026-01-27_14-10-48_screenshot.png}
\caption{\label{fig:org2e135b6}Trace plots and autocorrelation plots for a standard Gaussian initiailised with \(x = 0\) and using an RWM with a Gaussian proposal algorithm for 1000 iterations. The scale parameters used were 0.24, 0.26, and 24 respectively}
\end{figure}
